...
class PackagesSample {
  ...
  @Test
  def testPackageObject(): Unit = println(scala.imports.name)

  @Test
  def testImportAllMembers(): Unit = {
    import com.google.common.base.Preconditions._
    checkArgument(scala.imports.name.contains("package"), "%s does not contains package", "package object")
    checkNotNull(scala.imports.name, "%s is null", "package object")
  }

...
class RationalDesign {
  ...
  /*Note that implicit conversions are not applicable because they are ambiguous:
   both method intToRational in package objects of type (i: Int)scala.objects.Rational
   and method intToRationalConflict in class RationalDesign of type (i: Int)scala.objects.Rational
   are possible conversion functions from Int(2) to ?{def *(x$1: ? >: scala.objects.Rational): ?} */

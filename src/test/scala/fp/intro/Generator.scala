...
/**
  * from: https://github.com/lihaoyi/geny
...
trait Generator[+A] {
  ...
  def exists(f: A => Boolean): Boolean = find(f(_)).isDefined

  def contains(a: Any): Boolean = exists(_ == a)
  ...
  def flatten[V](implicit f: A => Generator[V]): Generator[V] = this.flatMap[V]((x: A) => f(x))

  def slice(start: Int, end: Int): Generator[A] = new Generator.Sliced(this, start, end)

  def take(n: Int): Generator[A] = slice(0, n)

  def drop(n: Int): Generator[A] = slice(n, Int.MaxValue)
  ...
  def toSet[B >: A]: Set[B] = toBuffer[B].toSet

  def toVector: Vector[A] = toBuffer.toVector
...
object Generator {
  ...
  private class ZipWithIndexed[+T](inner: Generator[T]) extends Generator[(T, Int)] {
    def generate(f: ((T, Int)) => Generator.Action): Action = {
      var i = 0
      inner.generate { t =>
        val res = f(t, i)
        i += 1
  ...
  private class Zipped[+T, V](inner: Generator[T], other: Iterable[V]) extends Generator[(T, V)] {
    def generate(f: ((T, V)) => Generator.Action): Action = {
